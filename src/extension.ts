...
function createTargets(body: string) {
	let str = body
		.replace(/'/g, '')
		.replace(/"/g, '')
		.trim();
...
export function activate(context: vscode.ExtensionContext) {
	...
	const disposable = vscode.commands.registerCommand('extension.convertEBNFToTypescript', function() {
		...
		if (editor) {
			...
			let validationMapEntries: string[] = [];
	...
	return entries.length === 0;
			...
			for (let i = 0; i < lines.length; i++) {
				...
				if (parts.every(isString)) {
					...
					const [r, fn, ve] = handleObject(title, body);
					res[i] = r;
					// @ts-expect-error
					functions.push(fn);
					validationMapEntries = [...validationMapEntries, ...ve];
				} else if (isDefinedKeyObject(body)) {
					// res[i] = handleDefinedKeyObject(title, body);
					const [r, fn, ve] = handleDefinedKeyObject(title, body);
					res[i] = r;
					// @ts-expect-error
					functions.push(fn);
					validationMapEntries = [...validationMapEntries, ...ve];
			...
			const result = `const validationMap = {${[...new Set(validationMapEntries)].join(',\n\t')}}` + '\n\n' + res.join('\n') + '\n\n' + functions.join('\n');
